...
/*
    server_name => service描述
                            =》service* 记录服务对象
                            method_name =》method方法对象
*/
// 这里是框架提供给外部使用的，可以发布到rpc方法的函数接口
...
{
    std::string nodeip = MprpcApplication::GetInstance().GetConfig().Load("rpcservernodeIp");
    uint16_t nodeport = atoi(MprpcApplication::GetInstance().GetConfig().Load("rpcservernodePort").c_str());
    muduo::net::InetAddress address(nodeip, nodeport);
    ...
    LOG_INFO("RpcProvider start service at ip:%s port:%d", nodeip.c_str(), nodeport);
